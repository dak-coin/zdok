...
*Setup instructions for a Gitian build of zdok Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the zdok
...
- [Building zdok Core](#building-zdok-core)
...
Clone the git repositories for zdok Core and Gitian.
...
in the zdok Core repository under 'Fetch and create inputs' to install sources which require
...
To build zdok Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the zdok Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zdok/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/zdokpay/zdok
...
For example:
```bash
URL=https://github.com/crowning-/zdok.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit zdok=${COMMIT} --url zdok=${URL} ../zdok/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zdok=${COMMIT} --url zdok=${URL} ../zdok/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit zdok=${COMMIT} --url zdok=${URL} ../zdok/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the zdok git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../zdok/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/zdok
SIGPATH=/some/root/path/zdok-detached-sigs

./bin/gbuild --url zdok=${BTCPATH},signature=${SIGPATH} ../zdok/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zdok-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zdok-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/zdok-osx-build.assert
```
...
[zdok/gitian.sigs](https://github.com/zdokpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
