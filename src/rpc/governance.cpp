...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in zdok.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("zdok.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* zdok features */
    { "zdok",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "zdok",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "zdok",               "gobject",                &gobject,                true,  {} },
    { "zdok",               "voteraw",                &voteraw,                true,  {} },
