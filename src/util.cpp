...
const char * const BITCOIN_CONF_FILENAME = "zdok.conf";
const char * const BITCOIN_PID_FILENAME = "zdokd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "zdok" is a composite category enabling all zdok-related debug output
                if(ptrCategory->count(std::string("zdok"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZdokCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZdokCore
    // Mac: ~/Library/Application Support/ZdokCore
    // Unix: ~/.zdokcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZdokCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ZdokCore";
#else
    // Unix
    return pathRet / ".zdokcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty zdok.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override zdok.conf
